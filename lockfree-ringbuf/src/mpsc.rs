...
///
/// Multiple threads can push concurrently, but only one thread can pop.
/// Uses atomic operations for coordination between producers.
...
impl<T: Copy> BatchOps<T> for MpscRingBuffer<T> {
